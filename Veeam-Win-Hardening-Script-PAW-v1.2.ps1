# -----------------------------------
# Veeam Windows Hardening Script v1.2 (Optimized for Windows Server 2022/2025)
...
function Set-RegistryValue {
    ...
    try {
        ...
        # Use New-ItemProperty -Force to set (or create) with the correct type consistently
        New-ItemProperty -LiteralPath $Path -Name $Name -PropertyType $Type -Value $Value -Force | Out-Null
        Write-Log ("Set registry: {0} -> {1}={2}" -f $Path, $Name, $Value) "success"
    } catch {
        Write-Log ("Failed to set registry: {0} -> {1}. Error: {2}" -f $Path, $Name, $_.Exception.Message) "error"
        throw
    }
}
...
Write-Log ("WinRE section consent: {0}" -f ($(if ($ApplyWinRE) {'granted'} else {'denied'})))
...
Write-Log ("===== Script finished at {0} =====" -f $Script:EndTime.ToString("yyyy-MM-dd HH:mm:ss.fff zzz")) "success"
